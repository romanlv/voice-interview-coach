...
export async function loadMetadata() {
  try {
    ...
    if (metadata.description && elements.pageDescription) elements.pageDescription.setAttribute('content', metadata.description);
    if (metadata.title && elements.headerTitle) elements.headerTitle.textContent = metadata.title;
    if (metadata.repository && elements.repoLink) elements.repoLink.href = metadata.repository;
  } catch (error) {
